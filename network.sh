...
CHAINCODE_COMMON_INIT='{"Args":["init","KDL218976","111111","VINOTH","CHALLA","0","ORD1111","M","FOXCONN-MEXICO","2017-12-12"]}'
...
function generateOrdererArtifacts() {
    ...
    configtxgen -profile OrdererGenesis -outputBlock ./artifacts/channel/genesis.block

    for channel_name in common "$ORG1-$ORG2" "$ORG1-$ORG3" "$ORG1-$ORG4" "$ORG2-$ORG3" "$ORG2-$ORG4" "$ORG3-$ORG4"
    do
        echo "Generating channel config transaction for $channel_name"
     configtxgen -profile "$channel_name" -outputCreateChannelTx "./artifacts/channel/$channel_name.tx" -channelID "$channel_name"
    done
...
function warmUpChaincode () {
    ...
    c="CORE_PEER_ADDRESS=peer0.$org.$DOMAIN:7051 peer chaincode query -n $n -v 1.0 -c '{\"Args\":[\"query\",\"KDL218976-111111\"]}'  -C $channel_name"
    i="cli.$org.$DOMAIN"
    echo ${i}
    echo ${c}
...
function devInstantiate () {
  docker-compose -f ${COMPOSE_FILE_DEV} run cli bash -c "peer chaincode instantiate -n mycc -v 0 -C myc -c '{\"Args\":[\"init\",\"KDL218976\",\"111111\",\"VINOTH\",\"CHALLA\",\"0\",\"ORD123212\",\"M\",\"FOXCONN-ME\",\"2012-12-12\"]}'"
}

function devInvoke () {
  docker-compose -f ${COMPOSE_FILE_DEV} run cli bash -c "peer chaincode invoke -n mycc -v 0 -C myc -c '{\"Args\":[\"move\",\"KDL218976\",\"111111\",\"VINOTH\",\"CHALLA\",\"0\",\"ORD123212\",\"M\",\"FOXCONN-ME\",\"2012-12-12\"]}'"
}

function devQuery () {
  docker-compose -f ${COMPOSE_FILE_DEV} run cli bash -c "peer chaincode query -n mycc -v 0 -C myc -c '{\"Args\":[\"query\",\"KDL218976-111111\"]}'"
}
...
function generatePeerArtifacts4() {
  generatePeerArtifacts ${ORG4} 4003 8084 9055 9052 9054 9057 9059
}
...
if [ "${MODE}" == "up" -a "${ORG}" == "" ]; then
 ...
 # createJoinInstantiateWarmUp ${ORG2} "${ORG2}-${ORG4}" ${CHAINCODE_BILATERAL_NAME} ${CHAINCODE_BILATERAL_INIT}
 ...
 # joinWarmUp ${ORG3} "${ORG3}-${ORG4}" ${CHAINCODE_BILATERAL_NAME}
  
  joinWarmUp ${ORG4} common ${CHAINCODE_COMMON_NAME}
 # joinWarmUp ${ORG4} "${ORG1}-${ORG4}" ${CHAINCODE_BILATERAL_NAME}
 # joinWarmUp ${ORG4} "${ORG2}-${ORG4}" ${CHAINCODE_BILATERAL_NAME}
 # joinWarmUp ${ORG4} "${ORG3}-${ORG4}" ${CHAINCODE_BILATERAL_NAME}
  ...
  downloadArtifactsMember ${ORG1} common "${ORG1}-${ORG2}" "${ORG1}-${ORG3}" "${ORG1}-${ORG4}"
 ...
 # createJoinInstantiateWarmUp ${ORG1} "${ORG1}-${ORG4}" ${CHAINCODE_BILATERAL_NAME} ${CHAINCODE_BILATERAL_INIT}
  ...
  downloadArtifactsMember ${ORG2} common "${ORG1}-${ORG2}" "${ORG2}-${ORG3}" "${ORG3}-${ORG4}"
...
#  createJoinInstantiateWarmUp ${ORG2} "${ORG2}-${ORG4}" ${CHAINCODE_BILATERAL_NAME} ${CHAINCODE_BILATERAL_INIT}
  ...
  downloadArtifactsMember ${ORG3} common "${ORG1}-${ORG3}" "${ORG2}-${ORG3}" "${ORG3}-${ORG4}"
 ...
 # joinWarmUp ${ORG3} "${ORG1}-${ORG3}" ${CHAINCODE_BILATERAL_NAME}
  ...
  downloadChannelBlockFile ${ORG3} ${ORG4} "${ORG4}-${ORG3}"
 #joinWarmUp ${ORG3} "${ORG4}-${ORG3}" ${CHAINCODE_BILATERAL_NAME}
elif [ "${MODE}" == "up-4" ]; then
  downloadArtifactsMember ${ORG4} common "${ORG1}-${ORG4}" "${ORG2}-${ORG4}" "${ORG3}-${ORG4}"
  ...
  downloadChannelBlockFile ${ORG4} ${ORG2} "${ORG2}-${ORG4}"
 # joinWarmUp ${ORG4} "${ORG2}-${ORG4}" ${CHAINCODE_BILATERAL_NAME}

  downloadChannelBlockFile ${ORG4} ${ORG1} "${ORG1}-${ORG4}"
  # joinWarmUp ${ORG4} "${ORG1}-${ORG4}" ${CHAINCODE_BILATERAL_NAME}
  
  downloadChannelBlockFile ${ORG4} ${ORG3} "${ORG3}-${ORG4}"
#  joinWarmUp ${ORG4} "${ORG3}-${ORG4}" ${CHAINCODE_BILATERAL_NAME}
elif [ "${MODE}" == "logs" ]; then
  logs ${ORG}
